...
{
    private static readonly ManualResetEventSlim InitEvent = new(false);
    ...
    public readonly Dictionary<ResearchProjectDef, Node> ResearchToNodesCache = [];
    ...
    public override void FinalizeInit()
    {
        ...
        {
            Assets.ColorCompleted[relevantTechLevels[i]] = Color.HSVToRGB(1f / count * i, 0.75f, 0.75f);
            Assets.ColorAvailable[relevantTechLevels[i]] = Color.HSVToRGB(1f / count * i, 0.33f, 0.33f);
            Assets.ColorUnavailable[relevantTechLevels[i]] = Color.HSVToRGB(1f / count * i, 0.125f, 0.33f);
        }
    }
    ...
    {
        Instance.Schedule(() => { Instance.Reset(); }, delayTicks: 600);
    }

    private Dictionary<TechLevel, IntRange> TechLevelBounds
    {
        get
        {
    ...
    private List<TechLevel> RelevantTechLevels
    {
        get
        {
            return _relevantTechLevels ??= (from TechLevel tl in Enum.GetValues(typeof(TechLevel))
    ...
    {
        var node = Instance.ResearchToNodesCache.TryGetValue(research);
        if (node != null)
        {
            return node;
        }

        _ = Instance.Nodes.OfType<ResearchNode>().ToList();
        return Instance.ResearchToNodesCache.TryGetValue(research);
    }
    ...
    private void Init()
    {
        ...
        {
            Logging.Error("Error initializing research tree. \n" + ex, true);
        }    
        finally
        {
            _initializing = false;
    ...
    public void Reset(bool newGame = false)
    {
        if (_initializing)
        {
            return;
    ...
    private void PopulateNodes()
    {
        ...
        var allDefsListForReading = DefDatabase<ResearchProjectDef>.AllDefsListForReading.Where(def => !def.IsHidden).ToList();
    ...
    public void ResetNodeAvailabilityCache()
    {
    ...
    private void DrawTechLevel(TechLevel techlevel, Rect visibleRect)
    {
    ...
    private bool CheckPrerequisitesInternal()
    {
        ...
        while (queue.Count > 0)
        {
            ...
            var enumerable = researchNode.Research.prerequisites
                .SelectMany(r => r.Ancestors())
                .ToList()
                .Intersect(researchNode.Research.prerequisites)
                .ToList();
            if (!enumerable.Any())
            {
                continue;
        ...
        while (queue.Count > 0)
        {
            ...
            returnValue = false;
            foreach (var researchNode in node.Children.Where(researchNode => !queue.Contains(researchNode)))
            {
                Logging.Warning(
            ...
            foreach (var researchNode in node.Parents.Where(researchNode => !queue.Contains(researchNode)))
            {
                Logging.Warning(
    ...
    private void HorizontalPositions()
    {
        var relevantTechLevels = RelevantTechLevels;
        var num = 1;
        var num2 = 50;
    ...
    private bool EdgeLengthSweep_Global()
    {
        var num = EdgeLength();
        for (var i = 2; i <= Size.x; i++)
        {
    ...
    private List<Node> Layer(int depth, bool ordered = false)
    {
